...
const BASE_DIR = CONFIG["CONFIG_BASE_DIR"]
    ? path.resolve(CONFIG["CONFIG_BASE_DIR"])
    : path.resolve("/srv/html/dl.kxtz.dev/files");
...
app.options(`/api/ping`, async (req, res) => res.status(200).json({ pong: "Pong!" }));
app.get(`/api/ping`, async (req, res) => res.status(200).json({ pong: "Pong!" }));
...
app.get(`/api/${CONFIG["CONFIG_APIVERSION"]}/download`, async (req, res) => {
    try {
        ...
        if (shouldThrottle) {
            ...
            const throttle = new Throttle({ rate: 5 * 1024 * 1024 });
...
app.get(`/api/version`, async (req, res) => {
    try {
        ...
        remoteInfoRaw.split("\n").forEach((line) => {
            ...
            if (type === "(fetch)" && !remotes[name]) remotes[name] = url;
        });
        ...
        const versionLine = `API ${CONFIG["CONFIG_APIVERSION"]}-${SERVER_START_TS}-g${commitHash}${isDirty ? "-dirty" : ""}`;
